...
interface LiUser {
  id: string; username: string; title?: string; online?: boolean; patron?: boolean;
  perfs: { bullet?: LiPerf; blitz?: LiPerf; rapid?: LiPerf; classical?: LiPerf };
  createdAt: number; seenAt?: number;
  playTime?: { total: number };
  count: { all: number; win: number; loss: number; draw: number };
  profile?: { country?: string; firstName?: string; lastName?: string; fideRating?: number };
}
...
const TC = [
  ...
  { label: "📅 Daily",  cc: "chess_daily",  li: "classical" as const },
];
...
export default function ExplorePlayer() {
  ...
  const search = async (e: React.FormEvent) => {
    ...
    const [profileRes, statsRes, liRes] = await Promise.allSettled([
      fetch(`https://api.chess.com/pub/player/${encodeURIComponent(lower)}`).then(r => r.ok ? r.json() : Promise.reject(r.status)),
      fetch(`https://api.chess.com/pub/player/${encodeURIComponent(lower)}/stats`).then(r => r.ok ? r.json() : Promise.reject(r.status)),
      fetch(`https://lichess.org/api/user/${encodeURIComponent(uname)}`, { headers: { Accept: "application/json" } }).then(r => r.ok ? r.json() : Promise.reject(r.status)),
    ]);
  ...
  const card = (hue: string, label: string, children: React.ReactNode) => (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", overflow: "hidden" }}>
      <div style={{ background: hue, borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "10px 16px" }}>
        <span style={{ fontWeight: 800, fontSize: "13px", color: "#e5e7eb" }}>{label}</span>
      </div>
    ...
    <ExploreLayout title="Player Lookup">
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "32px 20px 80px" }}>
        ...
        <div style={{ marginBottom: "28px" }}>
          ...
          <p style={{ color: "#9ca3af", fontSize: "13px" }}>Search any player on Chess.com and Lichess simultaneously.</p>
        </div>

        {/* Search bar */}
        <form onSubmit={search} style={{ display: "flex", gap: "10px", maxWidth: "480px", marginBottom: "32px" }}>
          <input
          ...
          />
          <button type="submit" disabled={!query.trim()} style={{ background: "#60a5fa", color: "#000", border: "none", borderRadius: "10px", padding: "12px 20px", fontWeight: 800, fontSize: "14px", cursor: "pointer", opacity: query.trim() ? 1 : 0.5 }}>
            Search
          </button>
        </form>
        ...
        {!searched && (
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "16px" }}>
            {["hikaru","magnuscarlsen","GMAnishGiri","LevonAronian"].map(u => (
        ...
        {searched && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }} className="player-grid">
            ...
            {/* Chess.com */}
            {card("rgba(129,211,119,0.1)", "♟ Chess.com", (
              <>
                ...
                {ccProfile && !ccLoading && (
                  <>
                    ...
                    <div style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "16px" }}>
                      {ccProfile.avatar
                        ? <img src={ccProfile.avatar} alt={ccProfile.username} width={52} height={52} style={{ borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(129,211,119,0.4)" }} />
                      ...
                      <div>
                        ...
                        {ccProfile.followers != null && <div style={{ color: "#6b7280", fontSize: "11px" }}>👥 {(ccProfile.followers).toLocaleString()} followers</div>}
                      </div>
                    </div>
            ...
            {/* Lichess */}
            {card("rgba(255,255,255,0.05)", "♞ Lichess", (
              <>
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "16px" }}>
                      <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px", position: "relative" }}>
                        ♞
                        {liUser.online && <span style={{ position: "absolute", bottom: 2, right: 2, width: 10, height: 10, borderRadius: "50%", background: "#4ade80", border: "2px solid #0d0404" }} />}
                      </div>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    ...
                    {/* Win/Draw/Loss */}
                    {liUser.count.all > 0 && (
                      <div style={{ display: "flex", gap: "4px", height: "6px", borderRadius: "999px", overflow: "hidden", marginBottom: "8px" }}>
                    ...
                    {/* Footer */}
                    <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: "10px", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
                      {liUser.playTime && <span style={{ color: "#9ca3af", fontSize: "11px" }}>🎮 {fmtTime(liUser.playTime.total)}</span>}
                      <span style={{ color: "#6b7280", fontSize: "11px" }}>📅 {fmtDate(liUser.createdAt, true)}</span>
